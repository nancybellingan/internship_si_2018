...
//============================================================================
	...
	G4ThreeVector worldSize = G4ThreeVector(5000.*cm, 5000.*cm, 1500.*cm);
	...
	G4LogicalVolume * logicWorld
				= new G4LogicalVolume(solidWorld, fAir, "World", 0, 0, 0);
...
G4Box* outerBox = new G4Box("Outer Box",outersize.x()/2,outersize.y()/2,outersize.z()/2);
    G4Box* innerBox = new G4Box("Inner Box",innersize.x()/2,innersize.y()/2,innersize.z()/2);
    G4SubtractionSolid* solidroom = new G4SubtractionSolid("solidroom",outerBox,innerBox);
// logic
...
/*// at surface phantom scorer
	G4ThreeVector  PMMAPhantomScorerSize = G4ThreeVector(30.*cm,30.*cm,1.*cm);
	G4ThreeVector  PMMAPhantomScorerPos  = G4ThreeVector(0.,0.,-14.5*cm);
	G4RotationMatrix* PMMAPhantomScorerRot = new G4RotationMatrix();

	fSolidPMMAPhantomScorer = new G4Box("solidPMMAPhantomScorer",
								PMMAPhantomScorerSize.x()/2.,
								PMMAPhantomScorerSize.y()/2.,
								PMMAPhantomScorerSize.z()/2.);

	fLogicPMMAPhantomScorer = new G4LogicalVolume(fSolidPMMAPhantomScorer,Air,
											"logicPMMAPhantomScorer",0,0,0);

	fPhysiPMMAPhantomScorer = new G4PVPlacement(PMMAPhantomScorerRot,
										PMMAPhantomScorerPos,
										fLogicPMMAPhantomScorer,
										"physiPMMAPhantomScorer",
										logicWorld,
										false,
										0,
										true);*/
...
//----------------------------------------------------------------------------
	...
	fUSDVolumes.push_back(fAlbedoZSens);
	fUSDNames.push_back(zADetVoxName);
